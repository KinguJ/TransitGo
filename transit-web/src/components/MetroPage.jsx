  ...
  useEffect(() => {
    const fetchMetroRoutes = async () => {
      try {
        ...
        // Filter metro vehicles first
        const metroVehicles = vehiclesRes.data.filter(v => v.type === 'Metro');
        
        // Get the line IDs that have metro vehicles
        const metroLineIds = new Set(metroVehicles.map(v => normalizeId(v.lineId)));
        
        // Filter lines that have metro vehicles
        const metroLines = linesRes.data.filter(line => 
          metroLineIds.has(normalizeId(line._id))
        );

        // Combine line and vehicle data
        const routes = metroLines.map(line => {
          const vehicle = metroVehicles.find(v => 
          ...
          return {
            ...
            nextDeparture: vehicle ? '3 min' : 'No vehicle assigned', // This should come from a schedule API
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <h2 className="text-xl font-semibold text-gray-900">Error loading metro routes</h2>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-gray-900">Metro Lines</h1>
              <p className="mt-2 text-sm text-gray-600">Live updates and schedules for all metro lines</p>
            </div>
            <Link
              to="/"
        ...
        {metroRoutes.length === 0 ? (
          <div className="bg-white shadow sm:rounded-lg p-6 text-center">
            <h3 className="text-lg font-medium text-gray-900">No metro lines available</h3>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center">
                        <div className="flex-shrink-0 h-12 w-12 bg-red-100 rounded-full flex items-center justify-center">
                          <span className="text-xl font-bold text-red-600">{route.routeNumber}</span>
                        </div>
                        <div className="ml-4">
